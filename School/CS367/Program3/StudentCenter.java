	...
	public static boolean readData(String fileName)
		...
		try	{
			...
			while(pointsPerStudent == 0)	{
				...
				if(nextLine.contains("Points/Student"))	{
			...
			while(!nextLine.contains("#Student"))	{
				courseList.add(new Course(lineScanner.next().trim(), lineScanner.next().trim(), lineScanner.nextInt()));
				nextLine = scan.nextLine();
			}
			
			//student shit. I'm done. fml. I'm gay.
		}
		catch(Exception e)
			{
			e.printStackTrace();
